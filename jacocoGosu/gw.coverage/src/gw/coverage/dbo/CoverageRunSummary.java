...
public class CoverageRunSummary {
	...
	// If we subtract, we store the difference of the averages. Therefore
	// runsAdded has to be one,
	...
	/**
	 * Update the average with another run. OR the line coverage bitmap with
	 * what we have.
	 *
	 * @param coverageRun
	 */
	public void addRun(CoverageRun coverageRun) {
		...
		logger.trace(title + ": adding coverageRun.instructionCovered " + instructionCovered + " to "
				+ instructionCovered + " with " + runsAdded + " so far");
	...
	/**
	 * Compare this run with another run. Subtract the otherRun and AND NOT the
	 * line coverage bitmap.
	 *
	 * @return a new coverage summary with the subtracted averages and the AND
	 *         NOT bits.
	 */
	public CoverageRunSummary subtract(CoverageRunSummary otherRun, String comparisonTitle) {
		if (this.runsAdded == 0) {
			...
			System.err.println(
					"Is there no data in the database for " + title + "? Comparing an empty summary " + toString());
		...
		if (otherRun.runsAdded == 0) {
			...
			System.err.println("Is there no data in the database for " + otherRun.title
					+ "? Comparing an empty summary " + otherRun.toString());
			return CoverageRunSummary.EMPTY_SUMMARY;
		}
	...
	public static String toCSVTitle() {
		return "title, " + "PackageName, " + "FileName, " + "lineMissed, " + "lineCovered, " + "instructionMissed, "
				+ "instructionCovered, " + "branchMissed, " + "branchCovered, " + "complexityMissed, "
				+ "complexityCovered, " + "methodMissed, " + "methodCovered, " + "runs" + ", "
				+ "line coverage difference cardinality";
	}

	public String toCSV(CoveredFile coveredFile) {
		return title + ", " + coveredFile.getPackageName() + ", " + coveredFile.getFileName() + ", " + getLineMissed()
				+ ", " + getLineCovered() + ", " + getInstructionMissed() + ", " + getInstructionCovered() + ", "
				+ getBranchMissed() + ", " + getBranchCovered() + ", " + getComplexityMissed() + ", "
				+ getComplexityCovered() + ", " + getMethodMissed() + ", " + getMethodCovered() + ", "
				+ (summaryRunsAdded != null ? summaryRunsAdded : runsAdded) + ", " + getCoveredLineSet().cardinality();
	}

	@Override
	public String toString() {
		if (runsAdded == 0) {
			return "CoverageRunSummary{" + "instructionMissed=n/a" + ", instructionCovered=n/a" + ", branchMissed=n/a"
					+ ", branchCovered=n/a" + ", lineMissed=n/a" + ", lineCovered=n/a" + ", complexityMissed=n/a"
					+ ", complexityCovered=n/a" + ", methodMissed=n/a" + ", methodCovered=n/a" + ", runsAdded="
					+ (summaryRunsAdded != null ? summaryRunsAdded : runsAdded) + ", summary="
					+ (summaryRunsAdded != null ? "yes" : "no") + ", coveredLineSet=\n"
					+ (coveredLineSet == null ? "null" : coveredLineSet.toString()) + "}\n";
		} else {
			return "CoverageRunSummary{" + "instructionMissed=" + getInstructionMissed() + ", instructionCovered="
					+ getInstructionCovered() + ", branchMissed=" + getBranchMissed() + ", branchCovered="
					+ getBranchCovered() + ", lineMissed=" + getLineMissed() + ", lineCovered=" + getLineCovered()
					+ ", complexityMissed=" + getComplexityMissed() + ", complexityCovered=" + getComplexityCovered()
					+ ", methodMissed=" + getMethodMissed() + ", methodCovered=" + getMethodCovered() + ", runsAdded="
					+ (summaryRunsAdded != null ? summaryRunsAdded : runsAdded) + ", summary="
					+ (summaryRunsAdded != null ? "yes" : "no") + ", coveredLineSet=\n"
					+ (coveredLineSet == null ? "null" : coveredLineSet.toString()) + "}\n";
		}
