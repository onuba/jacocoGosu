...
public final class AgentOptions {
	 ...
	 * Default is <code>null</code> (no dumps). This is useful for Gosu classes.
	 */
	...
	private static final Collection<String> VALID_OPTIONS = Arrays.asList(DESTFILE, APPEND, INCLUDES, EXCLUDES,
			EXCLCLASSLOADER, SESSIONID, DUMPONEXIT, OUTPUT, ADDRESS, PORT, CLASSDUMPDIR, JMX);
	...
	/**
	 * Gets the directory where we want to safe class files for which bytecode
	 * is dynamically generated. NULL if not specified, no class files will be
	 * safed.
	 *
	...
	private void setOption(final String key, final String value) {
		if (value.contains(",")) {
			throw new IllegalArgumentException(format("Invalid character in option argument \"%s\"", value));
		}
