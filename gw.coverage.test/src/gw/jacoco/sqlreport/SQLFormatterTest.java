...
public class SQLFormatterTest {
	...
	public void setup() throws Exception {
		...
		visitor = formatter.createVisitor(sqlSession, "branch", "changelist", "test suite",
				new SimpleDateFormat().parse("07/10/12 4:5 PM, PDT"));
	}
	...
	public void testStructureWithGroup() throws IOException {
		...
		assertEquals(
				"INSTRUCTION_MISSED=10 INSTRUCTION_COVERED=15 BRANCH_MISSED=1 BRANCH_COVERED=2 LINE_MISSED=0 LINE_COVERED=3 COMPLEXITY_MISSED=1 COMPLEXITY_COVERED=2 METHOD_MISSED=0 METHOD_COVERED=1 SUITE=test suite PACKAGE=org/jacoco/example CLASS=FooClass SUITE_RUN_DATE=2012-07-10 16:05:00.0 BRANCH=branch CHANGELIST=changelist ",
				lines.get(0));
	}
	...
	public void testSetEncoding() throws Exception {
		...
		visitor = formatter.createVisitor(sqlSession, "branch", "changelist", "test suite",
				new SimpleDateFormat().parse("11/11/12 4:5 PM, PDT"));
		driver.sendBundle(visitor);
		final List<String> lines = getLines("UTF-16");
		assertEquals(
				"INSTRUCTION_MISSED=10 INSTRUCTION_COVERED=15 BRANCH_MISSED=1 BRANCH_COVERED=2 LINE_MISSED=0 LINE_COVERED=3 COMPLEXITY_MISSED=1 COMPLEXITY_COVERED=2 METHOD_MISSED=0 METHOD_COVERED=1 SUITE=test suite PACKAGE=org/jacoco/example CLASS=FooClass SUITE_RUN_DATE=2012-11-11 16:05:00.0 BRANCH=branch CHANGELIST=changelist ",
				lines.get(0));
	}
	...
	public void testGetLanguageNames() throws Exception {
		ILanguageNames names = new ILanguageNames() {
			...
			@Override
			public String getQualifiedMethodName(String vmclassname, String vmmethodname, String vmdesc,
					String vmsignature) {
				return null;
			}
		};
	...
	private List<String> getLines(String encoding) throws IOException {
		...
		try {
			...
			ResultSet result = select.executeQuery("SELECT INSTRUCTION_MISSED, INSTRUCTION_COVERED, BRANCH_MISSED, "
					...
					+ "FROM PACKAGE_COVERAGE  sc " + "join package p on sc.package_id = p.id "
					+ "join changelist ch on sc.changelist_id = ch.id  " + "join class cl on sc.class_id=cl.id "
					+ "join branch b on sc.branch_id = b.id " + "join suite s on sc.suite_id = s.id\n");
			while (result.next()) {
				StringBuffer line = new StringBuffer();
				for (int i = 1; i <= result.getMetaData().getColumnCount(); i++) {
					logger.info(
							"result.getMetaData().getColumnName(" + i + ")=" + result.getMetaData().getColumnName(i));
