 ...
 * The tool would load the coverage data for the branches, changelists and
 * products specified on the command line. Then it needs to group them into
 * files, and do a bitwise OR for the non-PL and PL suites. Then it has to do an
 * (non-PL AND NOT PL) comparison with the results, to get the lines covered by
 * non-PL but not by PL. Finally the 1 bits have to be counted to arrive at the
 * "Lines covered by non-PL, not covered by PL" result column. Having the output
 * as CSV might be good for importing into Excel.
 */
public class SourceReport {
	...
	public static void main(final String[] args) throws IOException {
		...
		try {
			...
			System.err.println(
					"java -cp jacoco.jar;gw-coverage.jar;h2.jar -createTables -branch e-pr-merge -changelist 444222 -classesdir P:\\eng\\emerald\\pl\\ready\\active\\core\\gitmo\\configenv\\platform\\pl\\classes -execfile jc-coverage-gitmo-PLV3BareBone.exec -suite PLV3BareBone -jdbc jdbc:h2:~/coverage");
			System.exit(2);
		}

		final SourceReport report = new SourceReport();
		report.withBranchName(commandLineArguments.branchName).withChangelist(commandLineArguments.changeList)
				.withDbEnv(commandLineArguments.dbEnv).withSuiteRunDate(commandLineArguments.suiteRunDate)
				.withFilePattern(commandLineArguments.filePattern).withApps(commandLineArguments.apps);
		report.create();
	}
	...
	// using the parameters gathered query the database for matching file
	// records.
	private void create() {
	...
	private void initializeIBatis() {
		...
		try {
			...
			sessionFactory = new SqlSessionFactoryBuilder().build(resourceAsReader, "onuba");
	...
	public String toString() {
		return "SourceReport{" + "apps=" + apps + ", suiteRunDate=" + suiteRunDate + ", branchName='" + branchName
				+ '\'' + ", changelist='" + changelist + '\'' + ", filePattern='" + filePattern + '\'' + ", dbEnv='"
				+ dbEnv + '\'' + '}';
	}
}
